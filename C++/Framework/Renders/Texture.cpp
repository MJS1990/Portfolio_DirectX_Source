...
D3D11_TEXTURE2D_DESC Texture::ReadPixels(ID3D11Texture2D * src, DXGI_FORMAT readFormat, vector<D3DXCOLOR>* pixels)
{
	...
	UINT* colors = new UINT[desc.Width * desc.Height]; //픽셀 색상 저장할 변수
	...
	pixels->reserve(desc.Width * desc.Height); //공간 할당
	for (UINT y = 0; y < desc.Height; y++) //픽셀 색상값 pixels에 저장
	{
...
void Texture::SaveFile(wstring file, ID3D11Texture2D * src)
{
	...
	HRESULT hr;
	hr = D3D::GetDevice()->CreateTexture2D(&destDesc, NULL, &dest); //dest 텍스쳐 생성
	assert(SUCCEEDED(hr));

	hr = D3DX11LoadTextureFromTexture(D3D::GetDC(), src, NULL, dest); //src->dest복사
	assert(SUCCEEDED(hr));

	hr = D3DX11SaveTextureToFile(D3D::GetDC(), dest, D3DX11_IFF_PNG, file.c_str()); //텍스쳐 파일 생성
	assert(SUCCEEDED(hr));
...
void Textures::Load(Texture * texture, D3DX11_IMAGE_LOAD_INFO * loadInfo)
{
	...
	TexMetadata metaData; //불러온 텍스쳐 파일의 데이터 담을 구조체
	wstring ext = Path::GetExtension(texture->file); //확장자
	if (ext == L"tga")
	{
	...
	TextureDesc exist;
	bool bExist = false;
	for (TextureDesc temp : descs) //전에 Load한 텍스쳐 상태값이 존재한다면
	{
		if (desc == temp) //새로 로드하려는 텍스쳐파일이 전에 로드했던 파일이라면
		{
			bExist = true;
			exist = temp; //exist에 동일한 TextureDesc값 대입

			break;
		}
	}
	//이미 로드했던 텍스쳐가 존재한다면 텍스쳐 인스턴스에 설정값, view데이터 복사
	if (bExist == true) 
	{
	...
	{
		//ScratchImage - 이미지, 이미지의 집합을 반환하는 함수의 메모리를 관리하는 헬퍼 클래스
		ScratchImage image;
		if (ext == L"tga")
		{
...
//ShaderResourceView배열 srv에 데이터 담음(srv 생성)
