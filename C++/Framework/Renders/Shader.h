...
public:
	Shader(wstring file, bool bSubContext = false, bool bInstancing = false); //TODO : bSubContext 컴퓨트 쉐이더 관련 옵션
	~Shader();
	...
	void DrawIndexed(UINT technique, UINT pass, UINT indexCount, UINT startIndexLocation = 0, UINT baseVertexLocation = 0);
...
private:
	struct StateBlock
	{
		...
		ID3D11BlendState* OMBlendState; //OM에 바인딩
...
private:
	struct Pass
	{
		...
		//디바이스 상태 나타냄(각 파이프라인과 바인드되는 데이터들의 상태 변경을 bool로 표현)
