...
Terrain::Terrain()
{
	...
	//ReadHeightMap(Contents + L"HeightMaps/ground1_Height.dds");
	//ReadHeightMap(Textures + L"ground5_Height.dds");
	CalcNormalVector();
	CalcTangentVector();
	AlignVertexData();
...
bool Terrain::TerrainPicked(OUT D3DXVECTOR3 * out, Ray & ray) //OUT UINT * pickedIndex, 이전 마지막 파라메터값 const D3DXVECTOR3* tPosition
{
