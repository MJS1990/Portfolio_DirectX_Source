...
const UINT QuadTree::DivideCount = 2049; //1024, 32x32
...
void QuadTree::Initialize()
{
	...
	//material->SetDiffuseMap(Assets + L"TerrainTexture/rock_boulder_cracked_diff_4k.jpg");
	//material->SetNormalMap(Assets + L"TerrainTexture/rock_boulder_cracked_arm_4k.jpg");
	...
	//	perspectiveForCulling = new Perspective(desc.Width, desc.Height, (float)D3DX_PI * 0.6f); //0.6f
...
void QuadTree::RenderNode(NodeType * node)
{
	...
	//if((distance.x > 75.0f || distance.x < -75.0f) && distance.z > 30.0f) //원본값 20.0f
	//{
	//	if (frustum->ContainPoint(center) == false)
	//		return;
	//}
...
void QuadTree::CalcMeshDimensions(UINT vertexCount, float & centerX, float & centerZ, float & meshWidth)
{
	...
	//centerX, Z기준으로 나눴을때, 그 안에서 최대, 최소넓이, 깊이 찾는 부분
	for (UINT i = 0; i < vertexCount; i++)
	{
...
{	
	...
	csHeightTex = new Texture(Assets + L"/TerrainTexture/rock_boulder_cracked_disp_4k.png");
